 %% abffileSelectEnd_mean_image
%201231 EH. modified to replace Ffile with vector of mean image intensity
% tested on dopamine sensor data, GRAB-DA1h.
% here do not see localized transients, uses image intensity mean
%use after concatenate_tif_mean_EH
%follow with plot_imgMeans_rew_licks
%
...
%The code will try to extract the time of environment shifts automatically
%and will display the number of shifts detected and give you the option of manually accepted or rejecting each one.
close all
clear all;
[filename,path]=uigetfile('*.abf','pick your abf file');
cd (path); %set path
...
% if ~new
    ...
    figure;subplot(2,1,1);hold on;plot(data(:,5));plot(abs(diff(data(:,5)))>0.3*max(abs(diff(data(:,5)))),'r');
    figure;subplot(2,1,2);hold on;plot(data(:,5));plot(abs(diff(data(:,5)))>0.3*max(abs(diff(data(:,5)))),'r');
...
%     scanstop=1622465;%use if hand selecting stop point. Remember to re-comment when done!!!!!
    disp(['Length of scan is ', num2str(scanstop-scanstart)])
    disp(['Time of scan is ', num2str((scanstop-scanstart)/1000)])
    
    close all;
    
    ROE=data(scanstart:scanstop,4); %anglecut=view angle, channel 4 of "data"
...
%     rotationvel=data(scanstart:scanstop,7);  %120806 EH
    ...
    lick=data(scanstart:scanstop,3);
    if size(data,2)>5
        sol2=data(scanstart:scanstop,6);
    end
...
%     numframes=length(scanstart:scanstop);
%         lick=data(scanstart:scanstop,7); 
%         nullLick=constrain(abs([0;diff(angle)]))>.1;
%         nullLick2=constrain(abs([diff(angle);0]))>.1;
%         lick(nullLick>0)=0; lick(nullLick2>0)=0;
%        
%         
% end
...
for n=1:numfiles
    ...
    save(fullFfile,'ybinned','numframes','rewards','galvo','ROE','lick','sol2','-append'); %131018 added galvobinned
    %     if size(data,2)>7%131018
...
% %%
% %Identify rewards which are out of endzone - putative environment
% %transition points
% 
